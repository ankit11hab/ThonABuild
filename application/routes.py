    ...
    if current_user.is_authenticated:
        ...
        if form.validate_on_submit():
            ...
            if(request.files['image']):
                ...
                print(upload_result['secure_url'])
            from twilio.rest import Client
            notif = NotificationHistory(user_id=current_user.id, type='Event Based', email=form.sendmail.data, sms=form.sendsms.data, whatsapp=form.sendwhatsappmsg.data,
                                        message_body=template, period=form.frequency.data, scheduled_date=form.scheduled_date.data, score=form.score.data, event=form.event.data, activity=form.activity.data)
            db.session.add(notif)
            db.session.commit()
            data = current_user.data
    ...
    if current_user.is_authenticated:
        ...
        if form.validate_on_submit():
            ...
            if(request.files['image']):
                ...
                print(upload_result['secure_url'])
            from twilio.rest import Client
            notif = NotificationHistory(user_id=current_user.id, type='Activity Based', email=form.sendmail.data, sms=form.sendsms.data, whatsapp=form.sendwhatsappmsg.data,
                                        message_body=template, period=form.frequency.data, scheduled_date=form.scheduled_date.data, score=form.score.data, event=form.event.data, activity=form.activity.data)
            db.session.add(notif)
            db.session.commit()
            data = current_user.data
...
@app.route('/role_based', methods=['GET', 'POST'])
def role_based():
    if current_user.is_authenticated:
        form = NotifyTemplate()
        if form.validate_on_submit():
...
@app.route('/participation_based', methods=['GET', 'POST'])
def participation_based():
    if current_user.is_authenticated:
        form = NotifyTemplate()
        if form.validate_on_submit():
            template = form.template.data
...
def conditional_triggers():
    if current_user.is_authenticated:
        notif = current_user.history
        print(notif)
        score = activity = event = role = participation = 0
        for item in notif:
            if item.type == 'Score Based':
                ...
                activity += 1
            elif item.type == 'Role Based':
                role += 1
            elif item.type == 'Event Participation':
                participation += 1
